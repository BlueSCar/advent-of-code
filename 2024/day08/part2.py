...
for key in frequencies:
    ...
    for i in range(len(antennas) - 1):
        for j in range(i + 1, len(antennas)):
            ...
            while 0 <= point1[0] <= grid_height - 1 and 0 <= point1[1] <= grid_width - 1:
                antinodes.add(point1)
                point1 = (point1[0] + height_diff, point1[1] + width_diff)
                
            point2 = (antennas[j][0] - height_diff, antennas[j][1] - width_diff)
            while 0 <= point2[0] <= grid_height - 1 and 0 <= point2[1] <= grid_width - 1:
                antinodes.add(point2)
                point2 = (point2[0] - height_diff, point2[1] - width_diff)
